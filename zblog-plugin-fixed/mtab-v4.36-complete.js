/* ========================================
   MTAB v4.36 - Complete Merged Version
   ======================================== */
...
/* ========================================
   MTAB v4.36 - Complete Merged Version
   整合：lunar.js + sortable.min.js + txcstx.min.js + mtab.js
   每次修改版本号 +0.01
   ======================================== */
...
function txpostsubmit(a, h, f) {
    var formDom = $(f).closest('.tx-form-dom');
    if (formDom.length === 0) {
        formDom = $(f).closest('.del-box');
        if (formDom.length === 0) {
            formDom = $(f).closest('.cate-box');
        }
        if (formDom.length === 0) {
            formDom = $(f).closest('.article-box');
        }
    }
    if (formDom.length === 0) {
        formDom = $(f).parents('.tx-pop-txt').find('.tx-form-dom');
    }
    
    var O = formDom.find('input[name="name"]'), ag = formDom.find('input[name="url"]'), fM = formDom.attr('data-type');
    'undefined' !== fM && (a = fM);
    ...
    // 优先从表单里读取 cateID
    var formCateID = formDom.find('input[name="cateID"]').val();
    
    var postData = {
        name: formDom.find('input[name="name"]').val(),
        url: formDom.find('input[name="url"]').val(),
        img: formDom.find('input[name="img"]').val(),
        info: formDom.find('textarea[name="info"]').val(),
        order: formDom.find('input[name="order"]').val(),
        cateID: formCateID || $('#getid').val(),
        author: formDom.find('input[name="author"]').val(),
        token: formDom.find('input[name="token"]').val(),
        modify: formDom.find('input[name="modify"]').val(),
        icon: formDom.find('input[name="icon"]').val(),
        type: a,
        action: h
    };
    ...
    // 优先使用表单里的 cateID
    if (formCateID) {
        postData.cateID = formCateID;
    } else if ($('#getid').val()) {
        postData.cateID = $('#getid').val();
    }
    
    $.post(bloghost + 'zb_users/plugin/wx_nav/plugin/acceptdata.php', postData, function(data, status, xhr) {
        console.log('提交响应:', data, status);
        ...
        alert('提交失败: ' + error);
        $(f).val('提交').removeClass('disable').attr('disabled', false);
    })
}
...
function txPostIDpop(a, h, f) {
    ...
    if (fM === 'cate') {
        if (c === '1') {
            form.find('input[name="name"]').val($(a).attr('data-name') || '');
            form.find('input[name="order"]').val($(a).attr('data-sort') || '99');
            var iconVal = $(a).attr('data-icon') || 'icon-biaoqian';
            form.find('input[name="icon"]').val(iconVal);
            form.find('.icon-picker-selected i').attr('class', 'iconfont ' + iconVal);
            form.find('input[name="modify"]').val('on');
            if (form.find('input[name="cateID"]').length > 0) {
                form.find('input[name="cateID"]').val(ag);
            }
        } else {
            form.find('input[name="name"]').val('');
            form.find('input[name="order"]').val('99');
            form.find('input[name="icon"]').val('icon-biaoqian');
            form.find('.icon-picker-selected i').attr('class', 'iconfont icon-biaoqian');
            form.find('input[name="modify"]').val('');
            form.find('input[name="cateID"]').val('');
        }
    } else if (fM === 'post') {
        if (c === '1') {
            form.find('input[name="name"]').val($(a).parents('.set-item').find('.data-name').html());
            form.find('input[name="img"]').val($(a).parents('.set-item').find('.data-img').attr('src'));
            form.find('input[name="url"]').val($(a).parents('.set-item').find('a').attr('href'));
            form.find('textarea[name="info"]').val($(a).parents('.set-item').find('a').attr('title'));
            form.find('input[name="order"]').val($(a).attr('data-sort'));
            form.find('input[name="modify"]').val('on');
        } else {
            form.find('.tx-set-data').val('');
            form.find('input[name="order"]').val('99');
        }
    } else if (h.indexOf('删除') > -1) {
        if (form.find('input[name="cateID"]').length > 0) {
            form.find('input[name="cateID"]').val(ag);
        }
    }
    ...
    $('body').prepend('<div class="tx-pop"><div class="tx-pop-txt"><button class="tx-pop-off f-18">×</button>' + O + '<div class="tx-form-dom pd20" data-type="' + fM + '"></div></div><div class="tx-pop-off"></div></div>');
    var H = $(f);
...
function toggleSimpleMode() {
    ...
    var isSimpleMode = body.classList.contains('simple-mode');
    
    if (isSimpleMode) {
        body.classList.remove('simple-mode');
        zbp.cookie.set('simpleMode', '0', 365);
    } else {
        body.classList.add('simple-mode');
        zbp.cookie.set('simpleMode', '1', 365);
    }
}

function saveCurrentCategory(cid) {
    if (zbp.cookie.get('rememberCate') === '1') {
        zbp.cookie.set('lastCid', cid, 365);
    }
}

function restoreLastCategory() {
    if (zbp.cookie.get('rememberCate') === '1') {
        var lastCid = zbp.cookie.get('lastCid');
        if (lastCid) {
            ...
            if (found) {
                var url = tx_host + '?txact=list&cid=' + lastCid;
                txpostajax(url);
            } else {
                document.body.classList.remove('waiting-for-cate');
            }
        } else {
            document.body.classList.remove('waiting-for-cate');
        }
...
$(function() {
    $('.search-box #btnValidate').click(function() {
        ...
        var activeEngine = $('.engine-dropdown .engine-item.on');
        if (activeEngine.length > 0) {
            var url = activeEngine.attr('data-url');
            var ico = activeEngine.attr('data-ico');
            
            if (url) {
                if (ico === 'icon-liulanqi') {
                    window.location = url + keyword;
                } else {
                    var newUrl = url.replace('{$s}', encodeURIComponent(keyword));
                    window.open(newUrl);
                }
            }
        }
...
var timeSettings = {
    ...
    yearMonth: zbp.cookie.get('timeYearMonth') !== '0',
    lunar: zbp.cookie.get('timeLunar') !== '0',
    sexagenary: zbp.cookie.get('timeSexagenary') !== '0',
...
function initTimeSettings() {
    ...
    document.getElementById('timeYearMonth').checked = timeSettings.yearMonth;
    document.getElementById('timeLunar').checked = timeSettings.lunar;
    document.getElementById('timeSexagenary').checked = timeSettings.sexagenary;
...
function updateTimeSettings() {
    ...
    timeSettings.yearMonth = document.getElementById('timeYearMonth').checked;
    timeSettings.lunar = document.getElementById('timeLunar').checked;
    timeSettings.sexagenary = document.getElementById('timeSexagenary').checked;
    ...
    zbp.cookie.set('timeYearMonth', timeSettings.yearMonth ? '1' : '0', 365);
    zbp.cookie.set('timeLunar', timeSettings.lunar ? '1' : '0', 365);
    zbp.cookie.set('timeSexagenary', timeSettings.sexagenary ? '1' : '0', 365);
}
...
function updateTime() {
    ...
    if (timeSettings.lunar) {
        dateStr += (dateStr ? ' ' : '') + '农历' + lunar.monthStr + lunar.dayStr;
    }
    if (timeSettings.sexagenary) {
        dateStr += (dateStr ? ' ' : '') + lunar.yearGanZhi + '年';
    }
...
function bindTimeSettings() {
    var toggles = ['timeShow', 'time24h', 'timeSeconds', 'timeWeek', 'timeYearMonth', 'timeLunar', 'timeSexagenary'];
...
console.log('mtab.js 加载成功 v4.36');
    ...
    document.querySelectorAll('.sidebar-cate-item').forEach(function(item, index) {
        ...
        item.addEventListener('click', function(e) {
            ...
            if (this.classList.contains('sidebar-add-cate')) {
                console.log('点击了添加按钮，打开添加分类弹窗');
                txPostIDpop(this, '添加分类', '.cate-box');
                return;
            }
...
function toggleControlCenter(e) {
    ...
    var isVisible = cc.style.display === 'block';
    cc.style.display = isVisible ? 'none' : 'block';
...
function editCurrentItem(e) {
    ...
    var itemType = currentRightClickItem.dataset.type;

    if (itemType === 'cate' || currentRightClickItem.classList.contains('sidebar-cate-item')) {
        currentRightClickItem.dataset.set = '1';
        txPostIDpop(currentRightClickItem, '编辑分类', '.cate-box');
        ...
        setTimeout(function() {
            ...
            var info = currentRightClickItem.querySelector('a') ? currentRightClickItem.querySelector('a').title : '';

            var bgColor = $(currentRightClickItem).attr('data-bg-color') || '#ffffff';
            var itemId = currentRightClickItem.dataset.id || '';
            ...
            function fillData() {
                ...
                $tab.find('textarea[name="modal_info"]').val(info);
...
function deleteCurrentItem(e) {
    ...
    if (itemType === 'post' || itemType === 'cate') {
...
function changeWallpaper(n) {
    ...
    updateWallpaperPreview(bg);
}

function restoreDefaultWallpaper() {
    var bg = window.zbp_host + 'zb_users/plugin/wx_nav/style/img/bg1.jpg';
    document.body.style.backgroundImage = 'url(' + bg + ')';
    zbp.cookie.set('bgcolor', bg, 365);
    zbp.cookie.set('bgstyle', '2', 365);
    updateWallpaperPreview(bg);
...
function updateWallpaperPreview(bgUrl) {
...
function initWallpaperPreview() {
    ...
    // 2. 如果没有，尝试从内联样式或 CSS 获取
    if (!currentBg || currentBg === 'none') {
        var computedStyle = window.getComputedStyle(document.body);
        currentBg = computedStyle.backgroundImage;
    }
    
    // 3. 如果还是没有，从 cookie 获取
    if (!currentBg || currentBg === 'none') {
        currentBg = zbp.cookie.get('bgcolor');
        if (currentBg) {
            currentBg = 'url(' + currentBg + ')';
        }
    }
    
    // 4. 如果都没有，使用默认壁纸
    if (!currentBg || currentBg === 'none') {
        currentBg = 'url(' + window.zbp_host + 'zb_users/plugin/wx_nav/style/img/bg1.jpg)';
    }
    
    if (currentBg && currentBg !== 'none') {
        var urlMatch = currentBg.match(/url\(['"]?([^'"]+)['"]?\)/);
        if (urlMatch) {
            updateWallpaperPreview(urlMatch[1]);
        }
    }
}
...
function txpostsubmitCustom(form, type, action) {
    ...
    var postData = {
        ...
        info: $form.find('textarea[name="modal_info"]').val() || '',
        color: color,
...
function initIconPicker() {
    $(document).on('click', '.icon-picker-selected', function(e) {
        e.stopPropagation();
    ...
    $(document).on('click', '.icon-option', function(e) {
        ...
        var wrap = $(this).closest('.icon-picker-wrap');
        wrap.find('.icon-picker-selected i').attr('class', 'iconfont ' + icon);
        wrap.find('input[name="icon"]').val(icon);
        wrap.find('.icon-option').removeClass('selected');
        $(this).addClass('selected');
        wrap.removeClass('active');
    });
    
    $(document).click(function() {
        $('.icon-picker-wrap').removeClass('active');
    });
}

function initAddTabModal() {
    initColorPicker();
    initOnlineSearch();
    initCategoryTabs();
...
function getLunarDate(date) {
    try {
        if (typeof Solar !== 'undefined') {
            ...
            var monthStr = lunar.getMonthInChinese();
            var dayStr = lunar.getDayInChinese();
            var yearGanZhi = lunar.getYearInGanZhi();
        ...
        // Lunar.js error, silently fall back to simple version
    }
    ...
    var yearGanZhi = gan[ganIndex] + zhi[zhiIndex];
    ...
    return {
        ...
        isLeap: false,
        yearGanZhi: yearGanZhi,
        monthStr: lunarMonths[month - 1] + '月',
        dayStr: lunarDays[day - 1]
    };
}

function initTimePreview() {
    var timePreviewMain = document.querySelector('.time-preview-main');
    var timePreviewSub = document.querySelector('.time-preview-sub');

    function updateTimePreview() {
        if (!timePreviewMain || !timePreviewSub) return;
        ...
        var showSeconds = document.getElementById('timeSeconds')?.checked ?? true;
        var use24h = document.getElementById('time24h')?.checked ?? true;
        var showYearMonth = document.getElementById('timeYearMonth')?.checked ?? true;
        var showWeek = document.getElementById('timeWeek')?.checked ?? true;
        var showLunar = document.getElementById('timeLunar')?.checked ?? true;
        var showSexagenary = document.getElementById('timeSexagenary')?.checked ?? true;

        var displayHours = use24h ? hours : (hours % 12 || 12);
        var period = !use24h && hours >= 12 ? ' PM' : (!use24h && hours < 12 ? ' AM' : '');

        var timeStr = displayHours.toString().padStart(2, '0') + ':' + 
        ...
        timePreviewMain.textContent = timeStr;
        timePreviewSub.textContent = dateStr;
    }
...
function initBgSettings() {
    ...
    var bgMaskSlider = document.getElementById('bgMask');
    var bgBlur = document.querySelector('.bg-blur');
    var bgOverlay = document.querySelector('.bg-overlay');

    function updateBgBlur(value) {
        if (bgBlur) {
            bgBlur.style.filter = 'blur(' + value + 'px)';
        }
        zbp.cookie.set('bgBlur', value, 365);
    }

    function updateBgMask(value) {
        if (bgOverlay) {
            var opacity = value / 100;
            bgOverlay.style.background = 'rgba(0, 0, 0, ' + opacity + ')';
        }
        zbp.cookie.set('bgMask', value, 365);
    }

    if (bgBlurSlider) {
        bgBlurSlider.addEventListener('input', function() {
            updateBgBlur(this.value);
            updateSliderValue('bgBlur');
        });
    }

    if (bgMaskSlider) {
        bgMaskSlider.addEventListener('input', function() {
            updateBgMask(this.value);
            updateSliderValue('bgMask');
        });
    }

    var savedBlur = zbp.cookie.get('bgBlur');
    var savedMask = zbp.cookie.get('bgMask');

    if (savedBlur !== null) {
        bgBlurSlider.value = savedBlur;
        updateBgBlur(savedBlur);
    }

    if (savedMask !== null) {
        bgMaskSlider.value = savedMask;
        updateBgMask(savedMask);
    }
}

function initSidebarSettings() {
    var sidebar = document.querySelector('.mtab-sidebar');
    var sidebarEnabled = document.getElementById('sidebarEnabled');
    var sidebarAutoHide = document.getElementById('sidebarAutoHide');
    var rememberCate = document.getElementById('rememberCate');
    
    var sidebarTrigger = document.createElement('div');
    ...
    function saveSidebarSettings() {
        zbp.cookie.set('sidebarEnabled', sidebarEnabled.checked ? '1' : '0', 365);
        zbp.cookie.set('sidebarAutoHide', sidebarAutoHide.checked ? '1' : '0', 365);
        zbp.cookie.set('rememberCate', rememberCate.checked ? '1' : '0', 365);
    }

    function applySidebarEnabled() {
        if (sidebarEnabled.checked) {
            sidebar.style.transform = '';
            sidebar.style.opacity = '1';
            document.body.classList.remove('sidebar-hidden');
            if (sidebarAutoHide.checked) {
                setTimeout(resetHideTimer, 500);
            }
        } else {
            sidebar.style.transform = 'translateX(-100%)';
    ...
    function hideSidebar() {
        if (!sidebarAutoHide.checked || !sidebarEnabled.checked) return;
        sidebar.style.transform = 'translateX(-100%)';
        sidebar.style.opacity = '0';
        document.body.classList.add('sidebar-hidden');
    }
    ...
    function showSidebar() {
        if (!sidebarEnabled.checked) return;
        sidebar.style.transform = '';
        sidebar.style.opacity = '1';
        document.body.classList.remove('sidebar-hidden');
    }
    ...
    function resetHideTimer() {
        if (!sidebarAutoHide.checked) return;
    ...
    if (sidebarEnabled) {
        sidebarEnabled.addEventListener('change', applySidebarEnabled);
    }

    if (sidebarAutoHide) {
        sidebarAutoHide.addEventListener('change', function() {
            saveSidebarSettings();
            if (this.checked && sidebarEnabled.checked) {
                resetHideTimer();
    ...
    if (rememberCate) {
        rememberCate.addEventListener('change', function() {
            saveSidebarSettings();
        });
    }
    ...
    var savedEnabled = zbp.cookie.get('sidebarEnabled');
    var savedAutoHide = zbp.cookie.get('sidebarAutoHide');
    var savedRememberCate = zbp.cookie.get('rememberCate');

    if (savedEnabled !== null) {
        sidebarEnabled.checked = savedEnabled === '1';
        applySidebarEnabled();
    }

    if (savedAutoHide !== null) {
        sidebarAutoHide.checked = savedAutoHide === '1';
    }

    if (savedRememberCate !== null) {
        rememberCate.checked = savedRememberCate === '1';
    }

    if (sidebarAutoHide.checked && sidebarEnabled.checked) {
        setTimeout(resetHideTimer, 500);
    }
}
...
function initColorPicker() {
    ...
    if (colorPicker) {
        colorPicker.addEventListener('input', function() {
            ...
            $('.add-tab-modal .color-option').removeClass('active');
            
            var customOption = $('.add-tab-modal .color-option.color-picker-wrap');
            customOption.css('background', color);
...
function initOnlineSearch() {
    var searchInput = $('.add-tab-modal .search-input');
    var compCards = $('.add-tab-modal .comp-card');

    searchInput.on('input', function() {
        var keyword = $(this).val().toLowerCase().trim();
        
        compCards.each(function() {
            var title = $(this).find('.comp-title').text().toLowerCase();
            var desc = $(this).find('.comp-desc').text().toLowerCase();
...
function initCategoryTabs() {
    var catTabs = $('.add-tab-modal .cat-tab');
    var compCards = $('.add-tab-modal .comp-card');

    catTabs.on('click', function() {
        ...
        if (category === '全部') {
            ...
            compCards.hide();
            compCards.each(function() {
                var cardCategory = $(this).data('category') || '系统';
                if (cardCategory === category || category === '推荐') {
                    $(this).show();
                }
            });
        }
...
function initComponentButtons() {
    $(document).on('click', '.add-tab-modal .comp-btn', function() {
        var card = $(this).closest('.comp-card');
        var title = card.find('.comp-title').text();
        ...
        $(this).addClass('added').html('<i class="iconfont icon-duigou"></i>');
        txTips(title + '添加成功');
    });
}

function initWidgetButtons() {
    $(document).on('click', '.add-tab-modal .widget-btn', function() {
        var card = $(this).closest('.widget-card');
        var title = card.find('.widget-title').text();
        
        $(this).addClass('added').html('<i class="iconfont icon-duigou"></i>');
        txTips(title + '组件已添加到桌面');
    });

    initMuyuWidget();
}

function initMuyuWidget() {
    var muyuBox = $('.add-tab-modal .muyu-box');
    if (muyuBox.length === 0) return;

    var tapCount = 0;

    muyuBox.on('click', function() {
        tapCount++;
        
        $(this).addClass('muyu-active');
        setTimeout(function() {
            muyuBox.removeClass('muyu-active');
        }, 150);

        if (tapCount % 10 === 0) {
            txTips('功德+10，累计功德：' + tapCount);
        }
    });
}
...
function switchCateWithAnimation(url, direction) {
    ...
    if (mainInfo) {
        ...
        setTimeout(function() {
            ...
            var cidMatch = url.match(/cid=(\d+)/);
            if (cidMatch) {
                saveCurrentCategory(cidMatch[1]);
            }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('MTAB JS v4.36 开始初始化...');
    ...
    setTimeout(function() {
        ...
        // 初始化简洁模式
        var simpleMode = zbp.cookie.get('simpleMode');
        if (simpleMode === '1') {
    ...
    console.log('MTAB JS v4.36 加载完成 - 滚轮阈值已修复为 80，已整合 lunar.js 和 sortable.min.js');
});
